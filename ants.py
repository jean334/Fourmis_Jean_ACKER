...
if __name__ == "__main__":
    ...
    calc_plot = np.empty(nb_iter, dtype=np.float32)
    nourriture_plot = np.empty(nb_iter, dtype=np.int16)
    ...
    for i in range(nb_iter):
        ...
        # pg.time.wait(500)
        calc_plot[i] = end-deb
        nourriture_plot[i] = food_counter
        print(f"FPS : {1./(end-deb):6.2f}, nourriture : {food_counter:7d}", end='\r')


    inserer_valeur_dans_fichier_markdown("{}".format(np.mean(1./calc_plot)), "Compte_rendu_Fourmi-JEAN_ACKER.md", 
                                        r'\d*.*\d*', "<fps1>", "</fps1>")        
    inserer_valeur_dans_fichier_markdown("{}".format(nourriture_plot[-1]), "Compte_rendu_Fourmi-JEAN_ACKER.md", 
                                        r'\d*', "<nourriture1>", "</nourriture1>")    

    X = np.arange(1,nb_iter+1)
    selection = np.arange(1,nb_iter+1, 100)
    plt.title("temps de calcul total par itération")
    plt.ylabel("temps de calcul")
    plt.xlabel("itérations")
    plt.plot(X[selection], calc_plot[selection])
    plt.savefig("./ressources/fps_plot.png")
    
    plt.clf()

    plt.title("nourriture accumulée au cours du temps")
    plt.ylabel("nourriture")
    plt.xlabel("itérations")
    plt.plot(X, nourriture_plot)
    plt.savefig("./ressources/nourriture_plot.png")