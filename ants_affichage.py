...
"""
import numpy as np
import maze_affichage
from maze_affichage import Maze
import pheromone_affichage
from pheromone_affichage import Pheromon
...
class Colony:
    ...
    def explore(self, unloaded_ants, the_maze, pos_food, pos_nest, pheromones):
        ...
        has_north_exit = np.bitwise_and(the_maze.maze[old_pos_ants[:, 0], old_pos_ants[:, 1]], maze_affichage.NORTH) > 0
        has_east_exit = np.bitwise_and(the_maze.maze[old_pos_ants[:, 0], old_pos_ants[:, 1]], maze_affichage.EAST) > 0
        has_south_exit = np.bitwise_and(the_maze.maze[old_pos_ants[:, 0], old_pos_ants[:, 1]], maze_affichage.SOUTH) > 0
        has_west_exit = np.bitwise_and(the_maze.maze[old_pos_ants[:, 0], old_pos_ants[:, 1]], maze_affichage.WEST) > 0
...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        food_counter_buff = np.zeros(1, dtype=np.int16)

        temps_affichage = np.empty(nb_iter, dtype=np.float32)
    ...
    if rank == 1:
        ...
        pos_nest = 0, 0
        a_maze = maze_affichage.Maze(size_laby, 12345)
        ...
        pherom = pheromone_affichage.Pheromon(size_laby, pos_food, alpha, beta)
        food_counter = 0
        temps_calcul = np.empty(nb_iter, dtype=np.float32)
        temps_envoie = np.empty(nb_iter, dtype=np.float32)
        temps_total  = np.empty(nb_iter, dtype=np.float32)
        temps_affichage  = np.empty(nb_iter, dtype=np.float32)
        food_counter_buff = np.zeros(nb_iter, dtype=np.int16)


    for i in range(nb_iter):#while True:
        if rank == 0:
            ...
            Colony.display(screen, sprites, directions_buff, historic_path_buff, age_buff)
            pg.display.update()
            t2 = time.time()
            comm.Recv(food_counter_buff, source=1, tag=6)
            if food_counter_buff[0] == 1 and not snapshop_taken:
        ...
        if rank == 1:
            ...
            temps_calcul[i] = t3-deb
            temps_envoie[i] = end-t3
            temps_total[i] = end-deb
            food_counter_buff[i] = food_counter
            comm.Send(food_counter_buff[i], dest=0, tag=6)
    ...
    if rank==1:
        ...
        plt.title("Les différents temps de calcul par itération")
        plt.ylabel("temps de calcul")
        plt.xlabel("itérations")
        plt.plot(X[selection], temps_calcul[selection], label="temps de calcul par CPU1")
        plt.plot(X[selection], temps_envoie[selection], label="temps d'envoi des données")
        plt.plot(X[selection], temps_total[selection], label="temps total par itération")
        plt.plot(X[selection], temps_affichage[selection], label="temps d'affichage par itération")
        plt.legend()

        inserer_valeur_dans_fichier_markdown("{}".format(np.mean(1./temps_total)), "Rapport_Fourmis_Jean_ACKER.md", 
                                         r'\d*.\d*', "<fps2>", "</fps2>")  
        inserer_valeur_dans_fichier_markdown("{}".format(np.mean(temps_calcul)), "Rapport_Fourmis_Jean_ACKER.md", 
                                         r'\d*', "<tempscalcul2>", "</tempscalcul2>")    
        inserer_valeur_dans_fichier_markdown("{}".format(np.mean(temps_envoie)), "Rapport_Fourmis_Jean_ACKER.md", 
                                         r'\d*', "<tempsenvoie2>", "</tempsenvoie2>")    
        inserer_valeur_dans_fichier_markdown("{}".format(np.mean(temps_total)), "Rapport_Fourmis_Jean_ACKER.md", 
                                         r'\d*', "<tempstotal2>", "</tempstotal2>")    
        inserer_valeur_dans_fichier_markdown("{}".format(food_counter_buff[-1]), "Rapport_Fourmis_Jean_ACKER.md", 
                                         r'\d*', "<nourriture2>", "</nourriture2>")
        inserer_valeur_dans_fichier_markdown("{}".format(np.mean(temps_affichage)), "Rapport_Fourmis_Jean_ACKER.md", 
                                         r'\d*', "<affichage2>", "</affichage2>")

        plt.savefig("./ressources/fps_plot_affichage.png")
        """