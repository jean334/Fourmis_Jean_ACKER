...
class Maze:
    ...
    def __init__(self, dimensions, seed):
        ...
        #self.cases_img = []
        self.maze  = np.zeros(dimensions, dtype=np.int8)#c'est le tableau des sorties possibles pour chaque case
                                                        #les valeurs prisent sont comprisent entre 1 et 15
        ...
        historic.append(cur_ind)#historic contient l'hitorique des cases déja visitées mais dont il 
                                #reste des directions à explorer
        while (len(historic) > 0):
            ...
            neighbours         = [] #la liste des voisins de la case actuelle, c'est la liste V dans le notebook
            neighbours_visited = [] #la liste des voisins déjà visités
            direction          = [] # 
