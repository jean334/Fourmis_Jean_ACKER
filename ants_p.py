...
if __name__ == "__main__":
    ...
    directions_buff = None
    age_buff = None
    historic_path_buff = None
    food_counter_buff = None            
    temps_calcul = None
    temps_envoie = None
    temps_total  = None
    ...
    if rank >= 1:
        ...
        if rank == 1:
            ...
            nb_ants_per_processus = np.empty(nbp-1, dtype=np.int16)
            directions_buff = np.empty(total_ants, dtype=np.int16)
            historic_path_buff = np.empty(total_ants*(max_life+1)*2, dtype=np.int16)
            age_buff = np.empty(total_ants, dtype=np.int64)
    ...
    for i in range(nb_iter):#while True:
        if rank == 0:
            ...
            if food_counter_buff[0] == 1 and not snapshop_taken:
                pg.image.save(screen, "MyFirstFood_p.png")
                snapshop_taken = True
        ...
        if rank >= 1:
            deb = time.time()
            newComm.Allreduce([pherom.pheromon, MPI.FLOAT], [pherom_buff, MPI.FLOAT],  MPI.MAX) 
            newComm.Gatherv(sendbuf= ants.directions, 
                    recvbuf=(directions_buff, nb_ants_per_processus), root=0)
            print(nb_ants_per_processus)
            ...
            newComm.Reduce([np.array(food_counter, dtype=np.int32), MPI.INT], [food_counter_buff_unit, MPI.INT],root=0, op=MPI.SUM)
        ...
        if rank == 1:
            t4 = time.time()
            comm.Send(pherom_buff, dest=0, tag=0)
            comm.Send(directions_buff, dest=0, tag=2)
            comm.Send(historic_path_buff.reshape((total_ants, max_life+1, 2)), dest=0, tag=3)
            comm.Send(age_buff, dest=0, tag=4)
            t5 = time.time()
            comm.Send(food_counter_buff_unit, dest=0, tag=5)
